#ifndef SIFT_CUDA_UTIL_CUDA_CUH
#define SIFT_CUDA_UTIL_CUDA_CUH
...
public:
    ...
    explicit TimerGPU(cudaStream_t stream_ = nullptr) : stream(stream_) {
...
public:
    ...
    explicit TimerCPU(float freq_) : freq(freq_) {   // freq = clock frequency in MHz
        beg_clock = getTSC(bits);
    }

    static long long getTSC(int _bits) {
#ifdef WIN32
        return __rdtsc()/(1LL<<_bits);
#else
        unsigned int low, high;
        __asm__(".byte 0x0f, 0x31" :"=a" (low), "=d" (high));
        return ((long long) high << (32 - _bits)) | ((long long) low >> _bits);
#endif
    }
    ...
    [[nodiscard]] float read() const {
        ...
        float time = (float) (1 << bits) * (float) Kcycles / freq / 1e3f;
        return time;
    }
};
...
ShiftDown(T var, unsigned int delta, int width = 32) {
...
template<class T>
__device__ __inline__ T
ShiftUp(T var, unsigned int delta, int width = 32) {
#if (CUDART_VERSION >= 9000)
...
template<class T>
__device__ __inline__ T
Shuffle(T var, unsigned int lane, int width = 32) {
#if (CUDART_VERSION >= 9000)
