...
//    cv::cvtColor(img2, img2_gray, cv::COLOR_BGR2GRAY);
    ...
    std::cout << "detect time: " << t << "s, kpts: " << kpts1.size() << std::endl;
    ...
    cv::Mat desc;
    t = (double)cv::getTickCount();
    sift2->detect(img1_gray, kpts);
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    std::cout << "detect time: " << t << "s, kpts: " << kpts.size() << std::endl;

    t = (double)cv::getTickCount();
    sift2->compute(img1_gray, kpts, desc);
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    std::cout << "compute time: " << t << "s" << std::endl;
