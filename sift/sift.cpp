...
void SiftMultithreading::build_gaussian_pyramid(const cv::Mat &init_img, std::vector<std::vector<cv::Mat>> &gpyr,
                                                int nOctaves) const {
    const int nLayers = nOctaveLayers + 3;
    std::vector<double> sig(nLayers);
    gpyr.resize(nOctaves);
...
void SiftMultithreading::build_dog_pyramid(std::vector<std::vector<cv::Mat>> &dogpyr,
...
#pragma omp parallel for num_threads(omp_get_num_procs()) schedule(dynamic) default(none) shared(nOctaves, nLayers, gpyr, dogpyr)
	for (int i = 0; i < nOctaves; i++)
		for (int j = 0; j < nLayers; j++)
             cv::subtract(gpyr[i][j + 1], gpyr[i][j], dogpyr[i][j], cv::noArray(), CV_32FC1);
}
...
float
SiftMultithreading::calc_orientation_hist(const cv::Mat &image, cv::Point pt, float scale, int n, float *hist) const {
    int radius = cvRound(SIFT_ORI_RADIUS * scale);  //特征点邻域半径(3*1.5*scale)
	int len = (2 * radius + 1) * (2 * radius + 1);       //特征点邻域像素总个数（最大值）
	float sigma = SIFT_ORI_RADIUS*scale;                 //特征点邻域高斯权重标准差(1.5*scale)
	float exp_scale = -1.f / (2 * sigma * sigma);
...
void SiftMultithreading::find_scale_space_extrema(const std::vector<std::vector<cv::Mat>> &dogpyr,
    ...
    adjust(dogpyr, extrema, kpts, extrema_adjust);

    // calc_orientation
    calc_orientation(gpyr, kpts, extrema_adjust);
}

void SiftMultithreading::detect(const cv::Mat &image, std::vector<std::vector<cv::Mat>> &gpyr,
...
void SiftMultithreading::calc_sift_descriptor(const cv::Mat &gauss_image, float main_angle, cv::Point2f pt, float scale,
                                              int d, int n, float *desc) const {
    Sift::calc_sift_descriptor(gauss_image, main_angle, pt, scale, d, n, desc);
}

void SiftMultithreading::compute(const std::vector<std::vector<cv::Mat>> &gpyr, const std::vector<cv::KeyPoint> &keypoints,
...
#pragma omp parallel for num_threads(omp_get_num_procs()) schedule(dynamic) default(none) shared(kpts_num, kpts, gpyr, descriptors, d, n)
	for (size_t i = 0; i < kpts_num; ++i)//对于每一个特征点
	{
		//得到特征点所在的组号，层号
...
void SiftMultithreading::find_local_extrema(const std::vector<std::vector<cv::Mat>> &dogpyr, std::vector<LocalExtrema> &extrema) const {
    extrema.clear();
...
void
SiftMultithreading::adjust(const std::vector<std::vector<cv::Mat>> &dogpyr, const std::vector<LocalExtrema> &extrema,
...
void
SiftMultithreading::calc_orientation(const std::vector<std::vector<cv::Mat>> &gpyr, std::vector<cv::KeyPoint> &kpts,
                                     const std::vector<LocalExtrema> &extrema) const {
    int nKpts = kpts.size();
