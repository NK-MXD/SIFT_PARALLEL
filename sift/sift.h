...
class SiftMultithreading : public Sift
{
public:
    SiftMultithreading(int nfeatures = 0, int nOctaveLayers = 3, double contrastThreshold = 0.04,
                       double edgeThreshold = 10.0, double sigma = 1.6, int firstOctave = -1) :
    ...
    void calc_orientation(const std::vector<std::vector<cv::Mat>> &gpyr, std::vector<cv::KeyPoint> &kpts, const std::vector<LocalExtrema> &extrema) const;
};
