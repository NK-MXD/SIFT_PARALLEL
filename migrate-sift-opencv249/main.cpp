...
void test_detect() {
    ...
    sift->detect_avx(img_gray, gpyr, dogpyr, kpts);

    t = ((double)cv::getTickCount() - t) / getTickFrequency();
    std::cout << "MySift detect_avx time: " << t << " kpts size: " << kpts.size() << std::endl;
...
//        std::cout << "MySift detect time: " << t << " kpts size: " << kpts.size() << std::endl;
//
//        // 经过检查，在kpts.size() == 0时，img, img_gray, gpyr, dogpyr都是正常的
//        // 该bug与运行时有关
//        if (kpts.size() == 0) {
//            // check img, img_gray, gpyr, dogpyr
//            cv::imshow("img", img);
//            cv::imshow("img_gray", img_gray);
////            for (int i = 0; i < gpyr.size(); i++) {
////                for (int j = 0; j < gpyr[i].size(); j++) {
////                    cv::imshow("gpyr", gpyr[i][j]);
////                    cv::waitKey(0);
////                }
////            }
//            cv::Mat norm;
...
//                    cv::normalize(dogpyr[i][j], norm, 0, 255, cv::NORM_MINMAX, CV_8UC1);
//                    cv::imshow("dogpyr", norm);
//                    cv::waitKey(0);
//                }
//            }
//            cv::waitKey(0);
